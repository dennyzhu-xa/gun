...
@Repository
public class PerforatingProjectileLogDAOImpl extends AbstractDAOImpl<PerforatingProjectileLog> implements PerforatingProjectileLogDAO {
	private static final Log log = LogFactory.getLog(PerforatingProjectileLogDAOImpl.class);

	private String prepareSQLForEquipmentLog(){
		String sql = "SELECT id, perforating_id as \"perforatingId\", log_type as \"logType\", "
				...
				+ "modify_date as \"modifyDate\" FROM perforating_projectile_log where 1=1";
		return sql;
	}
	
	public List<PerforatingProjectileLogDTO> queryList(String equipId){
		List<PerforatingProjectileLogDTO> equipmentLogDTOList = null;
		...
		try{
			...
			if(StringUtils.hasText(equipId)){
				sqlQuery.setParameter(PerforatingProjectileLogDTO.ATTRIBUTE.PERFORATING_ID.getValue(), Integer.valueOf(equipId).intValue());
			}
			sqlQuery.setResultTransformer(Transformers.aliasToBean(PerforatingProjectileLogDTO.class));
