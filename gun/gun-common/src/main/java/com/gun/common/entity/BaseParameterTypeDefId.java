package com.gun.common.entity;
// Generated 2017-11-28 16:57:12 by Hibernate Tools 4.0.1.Final

/**
 * BaseParameterTypeDefId generated by hbm2java
 */
public class BaseParameterTypeDefId implements java.io.Serializable {


  private static final long serialVersionUID = 8545927389853489101L;
  
  private String bptdCode;
  private String effectiveDate;

  public BaseParameterTypeDefId() {
  }

  public BaseParameterTypeDefId(String bptdCode, String effectiveDate) {
    this.bptdCode = bptdCode;
    this.effectiveDate = effectiveDate;
  }

  public String getBptdCode() {
    return this.bptdCode;
  }

  public void setBptdCode(String bptdCode) {
    this.bptdCode = bptdCode;
  }
  public String getEffectiveDate() {
    return this.effectiveDate;
  }

  public void setEffectiveDate(String effectiveDate) {
    this.effectiveDate = effectiveDate;
  }

  public boolean equals(Object other) {
    if ((this == other))
      return true;
    if ((other == null))
      return false;
    if (!(other instanceof BaseParameterTypeDefId))
      return false;
    BaseParameterTypeDefId castOther = (BaseParameterTypeDefId) other;

    return ((this.getBptdCode() == castOther.getBptdCode())
            || (this.getBptdCode() != null && castOther.getBptdCode() != null && this.getBptdCode().equals(castOther.getBptdCode())))
            && ((this.getEffectiveDate() == castOther.getEffectiveDate()) || (this.getEffectiveDate() != null && castOther.getEffectiveDate() != null
                    && this.getEffectiveDate().equals(castOther.getEffectiveDate())));
  }

  public int hashCode() {
    int result = 17;

    result = 37 * result + (getBptdCode() == null ? 0 : this.getBptdCode().hashCode());
    result = 37 * result + (getEffectiveDate() == null ? 0 : this.getEffectiveDate().hashCode());
    return result;
  }

}
